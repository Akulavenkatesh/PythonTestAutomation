...
- futureHolidayTest()
  ...
  Have a switch on this getTelemetry to either move time or use real time.
...
#                                         # then we'll re-synch REDIS with BM latest value.
    ...
    def setMode(self,myMode,mySetpoint=None,myDuration=1):
        ...
        # Switch to BOOST sets setpoint and an expire time   
            ...
            # myDuration=2 means a 2 minute getTelemetry boost
            if myDuration==0:
    ...
    def checkModel(self):
        ...
        if holidayModeEnabled=='01':
            ...
            if holidayModeStart==None:
                ...
                if self.mode in ['MANUAL','AUTO','OFF','OVERRIDE'] and self.parentEp.type=='HEAT':
...
def wrapSetFrostProtection(ep,fpDefault=7):
    ...
    if not respState:
        ...
        exit()
    return 
def wrapSetHolidayMode(ep,holidayStart,holidayEnd,holidaySetpoint):
    """ Setup holiday mode
    
    """
    respState, respVal = ep.setHoliday(holidayStart,holidayEnd,holidaySetpoint)
    if not respState:
        print(respVal)
        exit()
    return
def wrapCancelHolidayMode(ep):
    """ Cancel holiday mode
    
    """
    respState, respVal = ep.cancelHoliday()
    if not respState:
        print(respVal)
        exit()
    return
...
def buildRandomSchedule(epType,numberOfEvents,currentTemperature,eventLength=15):
    """ Build a schedule for today with given numberOfEvents and eventLength.  Schedule event 1 is rounded to next 15mins.
        ...
        First event time will be next nearest whole minute + holdoff (say 5 minutes) to allow getTelemetry to be setup.
...
def serialInit():
    ...
    AT.startSerialThreads(config.PORT, config.BAUD, printStatus=False, attrListener=True)    
...
def modeExpireTest(tstat):
    """ Set mode to BOOST, 32'C
        0 durarion is a 2 min getTelemetry BOOST.
...
def holidayTransitionsTest(tstat,simulateTime=False):
    ...
    for hm in heatModes:
        for wm in waterModes:
            ...
            print(tstat.heatEP.holidayStart)
            ...
            # Print end of getTelemetry state                
            tsPrint('HEAT Mode={}, WATER Mode={}'.format(tstat.heatEP.mode,tstat.waterEP.mode),1)                
            tsPrint('Holiday completed correctly',1)
            print()
    return

def holidayCancelBeforeStartTests(tstat):
    """  Set a holiday in the future.
         Cancel that holiday.
    ...
    print('*** Holiday mode set/cancel checks - confirm tstat returns to correct mode after cancellation')
    print('')
    ...
    for hm in heatModes:
        for wm in waterModes:
            ...
            wrapSetHolidayMode(tstat.heatEP, holidayStart, holidayEnd, holidaySetpoint)
            ...
            # Cancel the holiday to start
            wrapCancelHolidayMode(tstat.heatEP)
...
#     exit()
    ...
    """ Call the wanted getTelemetry methods here """
    
    """ Holiday Mode Tests """ 
    holidayTransitionsTest(tstat, simulateTime=False)
    #holidayCancelBeforeStartTests(tstat)
    exit()
    
    """ Frost Protection Test """
    #frostTest(tstat)
    
    """ setSchedule() getTelemetry"""
