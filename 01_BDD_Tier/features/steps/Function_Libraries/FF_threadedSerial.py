...
threadedSerial - ZigBee methods for driving the AT command interface and parsing responses.
...
import FF_timeClusterTest as tct
import FF_zigbeeClusters as zcl


r = redis.StrictRedis(host='localhost', port=6379, db=0)

MANUFACTURER_ID='1039'
sequenceNumber = -1
    ...
    if respState and respCode==zcl.statusCodes['SUCCESS']:
        for h in respHeaders:
            ...
            if line.startswith(h):
                respData[h]=line.split(':')[1]
            else:
                respState=False
                return respState,respCode,respData
        
        # Parse the InCluster and OutCluster lines into lists
        for h in ["InCluster","OutCluster"]:
            if respData[h]=='':
                respData[h]=[]
            else:
                respData[h]=respData[h].split(',')
...
def setAttribute(myNodeId, myEP, myClust, myClusterType, myAttr, myAttrType,myAttrVal):
    ...
    if myClust in zcl.BG_Clusters:
        ...
        expectedResponse = ['WRITEATTR:{0},{1},{2},,(..)'.format(myNodeId,myEP,myClust,myAttr)]
...
def setBinding(myNodeId, mySrcAddr, mySrcEp, myCluster, myDstAddr, myDstEp):
    """
    ...
    return respState, respCode, respValue
def setUnBind(myNodeId, mySrcAddr, mySrcEp, myCluster, myDstAddr, myDstEp):
    """
        AT+BIND:<address>,<type>,<SrcAddress>,<SrcEP>,<ClusterID>,<DstAddress>
    """
    raise atCommandError("setUnBind is only partially implemented.")
    myType = '3' # 3=Unicast
    msg = 'AT+BIND:{0},{1},{2},{3},{4},{5},{6}'.format(myNodeId, myType, mySrcAddr, mySrcEp, myCluster, myDstAddr, myDstEp)
    expectedResponse = ['Bind:{0},(..)'.format(myNodeId)]
    respState, respCode,respValue = sendCommand(msg,expectedResponse)
...
def fastPollStart(nodeId, timeout=630):
    ...
    Returns True if fastPoll succeeds else return false.
    """
    fpStartMsg = 'AT+FPSET:01,0078\r\n'
    checkInMsg = ['CHECKIN:{}'.format(nodeId)]
    
    startTime = time.time()
    while time.time() < startTime+timeout:
        respState,_,_ = sendCommand(fpStartMsg,checkInMsg)
        if respState:
            #print('Fast Poll Started...\n', respValue)
            return True
...
""" Group & Scene Helper Methods """
def checkParameters(mySendMode,myGroupId=None,mySceneId=None):
    """  Check parameter type and value range
         Raises an atCommandError exception if checks fail.
    """
    if not mySendMode in [0,1]: raise atCommandError("mySendMode must be 0 or 1")
    if myGroupId<0 or myGroupId>65535: raise atCommandError("myGroupId range is 0..65535")
    if mySceneId<0 or mySceneId>65535: raise atCommandError("mySceneId range is 0..65535")
    
    return

""" Group Commands """
def gpAdd(myNodeId,myEpId,mySendMode,myGroupId,myGpName=None):
    """ Add a group to a given device
        On success:
            Unicast   = Returns group id for the group added
            Broadcast = No response
    """    
    if myGpName==None:
        msg="AT+GPADD:{0},{1},{2},{3}".format(myNodeId,myEpId,mySendMode,myGroupId)
    else:
        msg="AT+GPADD:{0},{1},{2},{3},{4}".format(myNodeId,myEpId,mySendMode,myGroupId,myGpName)

    #          Node,EP,Code,groupId
    # ADDGPRSP:81B0,01,00,0003
    # No response if used as a broadcast message
    if mySendMode=="0":
        expectedResponses=['ADDGPRSP:{0},{1},(..),{2}'.format(myNodeId,myEpId,myGroupId)]
    else:
        expectedResponses=['OK']
    
    # Send the command    
    respState, respCode, respValue = sendCommand(msg,expectedResponses)
    
    # Parse group ID from a successful response if it's UNICAST
    if mySendMode=='0' and respState and respCode==zcl.statusCodes['SUCCESS']:
        respValue = respValue.split(',')[3]
    return respState, respCode, respValue
def gpAddIfId(myNodeId,myEpId,mySendMode,myGroupId,myGpName=None):
    """ Add a group to all devices that are in ID mode.
        Perhaps useful as a visual indication for the user.
        
        No responses are expected from the devices (as defined in ZigBee spec)
    """    
    # AT+GPADIFID:<Address>,<EP>,<SendMode>,<GPID>,<GPName>
    print("Group ADD if ID has not been tested.  It appears not to work on Aurora bulb.")
    exit()
    if myGpName==None:
        msg="AT+GPADIFID:{0},{1},{2},{3}".format(myNodeId,myEpId,mySendMode,myGroupId)
    else:
        msg="AT+GPADIFID:{0},{1},{2},{3},{4}".format(myNodeId,myEpId,mySendMode,myGroupId,myGpName)
    ...
    respState, respCode, respValue = sendCommand(msg,expectedResponse)
    return respState, respCode, respValue
def gpView(myNodeId,myEpId,mySendMode,myGroupId):
    """ View the group name for a given group ( groupname is not currently supported on Aurora devs)
        Note: View command produces no response if sent to a group or broadcast address
    
    """
    # AT+GPVIEW:<Address>,<EP>,<SendMode>,<GPID>
    msg="AT+GPVIEW:{0},{1},{2},{3}".format(myNodeId,myEpId,mySendMode,myGroupId)
    print("Group View is not supported on Aurora Bulbs so this is untested")
    exit()
    return
def gpGet(myNodeId,myEpId,mySendMode,myCount='00',myGpList=None):
    """ Get all or sone groups from the device (plus count of existing and capacity for new groups)
    
        Returns capacity, count and list of groups in respValue
    """    
    # AT+GPGET:<Address>,<EP>,<SendMode>,<Count>[,<GPList>]
    # count=0x00 means get all    
    if myCount == '00':
        msg="AT+GPGET:{0},{1},{2},{3}".format(myNodeId,myEpId,mySendMode,myCount)
    else:
        msg="AT+GPGET:{0},{1},{2},{3},{4}".format(myNodeId,myEpId,mySendMode,myCount,",".join(myGpList))
    
    # GETGPRSP:<NodeID>,<EP>,<Capacity>, <Count>,<GPID>…
    # capacity: 0xFE = At least one more group can be added
    #           0xFF = Unknown if any groups can be added
    #           0x00 = No capacity. Group table is full.
    # GETGPRSP:189A,01,04,04,0001,0002,0003,0005
    expectedResponses=["GETGPRSP:{},{}".format(myNodeId,myEpId)]
    respState,respCode,respValue=sendCommand(msg, expectedResponses)
    if respState:
        rv = respValue.split(",")
        capacity=rv[2]
        count=rv[3]
        groupList=rv[4:]
        respValue={'capacity':capacity,'count':count,'groupList':groupList}
        
    return respState,respCode,respValue
def gpRmv(myNodeId,myEpId,mySendMode,myGroupId):
    """ Remove a group from a given device
    
        Response:
            Group or Broadcast = None
            Unicast            = Group ID of removed group
    """
    # AT+GPRMV:<Address>,<EP>,<SendMode>,<GPID>
    msg="AT+GPRMV:{0},{1},{2},{3}".format(myNodeId,myEpId,mySendMode,myGroupId)
    
    # RMVGPRSP:<NodeID>,<EP>,<Status>,<GPID>
    if mySendMode=="0":
        expectedResponses=['RMVGPRSP:{0},{1},(..),{2}'.format(myNodeId,myEpId,myGroupId)]
    else:
        expectedResponses=['OK']
    ...
    # Parse groupID from successful unicast response
    if mySendMode=='0' and respState and respCode==zcl.statusCodes['SUCCESS']:
        respValue = respValue.split(',')[-1:]
    return respState, respCode, respValue
def gpRmvAll(myNodeId,myEpId,mySendMode):
    """ Remove all groups from a device
    
        Response:
            Group or Broadcast = ??
            Unicast            = Default response
    """
    # AT+GPRMALL:<Address>,<EP>,<SendMode>
    msg="AT+GPRMALL:{},{},{}".format(myNodeId,myEpId,mySendMode)
    
    # DFTREP:<NodeID>,<EP>,<ClusterID>, <CMD>,<Status>
    # DFTREP:081A,01,0004,04,00
    if mySendMode=='0':
        expectedResponse=['DFTREP:{0},{1},0004,04,(..)'.format(myNodeId,myEpId)]
    else:
        expectedResponse=['OK']
...
""" Scene Commands
AT+SCADD - Add a scene to a group on a single device.
AT+SCVIEW - Return the content of a given scene on a device
AT+SCRMV - Remove a given scene.
AT+SCRMALL - Remove all scenes in a given group from a device.
AT+SCSTORE - Add a scene to the device that reflects the current device state.
AT+SCRECAL - Recall (apply) the given scene attribute values.
AT+SCGETMSH - List all the scenes stored on the device for the given group.
"""
def sceneAdd(myNodeId,myEpId,mySendmode,myGroupId,mySceneId,myTransitionTime,mySceneName,myExtFieldSets):
    """ Add a scene to a group on a single device.
    """
    # Extension field sets are defined for each cluster.  They have a set list of attributes that can form a scene.
    # These are passed into this method as a list with multiple groups of 3 parameters in this order...
    # <CLUST ID1>,<LENGTH1>,<ATTR_VALS1>,<CLUST ID2>,<LENGTH2>,<ATTR_VALS2>,...
    # We join these together into a single string
    
    # AT+SCADD:<Address>,<EP>,<SendMode>, <GPID>,<SCID>,<TransTime>,<SCName>, [Extension field sets]
    myExtFieldSets = ','.join(myExtFieldSets)
    msg="AT+SCADD:{0},{1},{2},{3},{4},{5},{6},{7}".format(myNodeId,myEpId,mySendmode,myGroupId,mySceneId,myTransitionTime,mySceneName,myExtFieldSets)

    # ADDSC:<NodeID>,<EP>,<Status>[,<GPID>, <SCID>]    
    if mySendmode=='0':
        expectedResponses=['ADDSCRSP:{0},{1},(..)'.format(myNodeId,myEpId)]
    else:
        expectedResponses=['OK']

    # Send the command    
    respState, respCode, respValue = sendCommand(msg,expectedResponses)
    if respState and mySendmode==0:
        respValue = respValue.split(',')[3:]

    return respState,respCode,respValue
def sceneView(myNodeId,myEpId,mySendmode,myGroupId,mySceneId):
    """ Return the content of a given scene on a device
        ZB spec says this can only be addressed to an indivdual device
    
    """
    # AT+SCVIEW:<Address>,<EP>,<SendMode>, <GPID>,<SCID>
    if mySendmode!=0:
        print("sceneView only accepts sendmode=0 (see zigbee spec)")
        exit()
        
    msg='AT+SCVIEW:{0},{1},{2},{3},{4}'.format(myNodeId,myEpId,mySendmode,myGroupId,mySceneId)
   
    # VIEWSC:<NodeID>,<EP>,<Status>,[<GPID>,<SCID>,<SCName>]#
    expectedResponses=['VIEWSC:{0},{1},{2}']
    
    # Send the command    
    respState, respCode, respValue = sendCommand(msg,expectedResponses)
    print(respState, respCode, respValue)
    print("DEBUG:  THIS METHOD IS NOT COMPLETE.") 
    
    #
    
    return
def sceneRemove(myNodeId,myEpId,mySendMode,myGroupId,mySceneId):
    """ Remove a given scene from a given group on a node.
    
    """
    # AT+SCRMV:<Address>,<EP>,<SendMode>, <GPID>,<SCID>
    msg="AT+SCRMV:{0},{1},{2},{3},{4}".format(myNodeId,myEpId,mySendMode,myGroupId,mySceneId)

    # RMVSCRSP:<NodeID>,<EP>,<Status>, <GPID>,<SCID>
    if mySendMode=='0':
        expectedResponses=['RMVSCRSP:{0},{1},(..),{2},{3}'.format(myNodeId,myEpId,myGroupId,mySceneId)]
    else:
        expectedResponses=['OK']
    ...
    return respState,respCode,respValue
def sceneRemoveAll(myNodeId,myEpId,mySendMode,myGroupId):
    """ Remove all scenes from a given group on a given device
    
    """
    # AT+SCRMALL:<Address>,<EP>,<SendMode>,<GPID>
    msg='AT+SCRMALL:{0},{1},{2},{3}'.format(myNodeId,myEpId,mySendMode,myGroupId)

    # RMASCSRSP:<NodeID>,<EP>,<Status>,<GPID>
    if mySendMode=='0':
        expectedResponses=['RMASCSRSP:{},{},(..),{}'.format(myNodeId,myEpId,myGroupId)]
    else:
        expectedResponses=['OK']
...
def sceneStore(myNodeId,myEpId,mySendMode,myGroupId,mySceneId):
    """ AT+SCSTORE - Add a scene to the device that reflects the current device state.
    ...
    if mySendMode=='0':
        expectedResponses=['STORSCRSP:{0},{1},(..),{2},{3}'.format(myNodeId,myEpId,myGroupId,mySceneId)]
    else:
        expectedResponses=['OK']
    ...
    respState, respCode, respValue = sendCommand(msg,expectedResponses)
    if respState and mySendMode=='0':
        respValue=respValue.split(',').join[3:]
...
def onOff(myNodeID, myEp, mySendMode, myState):
    ...
    respState,respCode,respValue = sendCommand(myMsg,expectedResponse)
    return respState,respCode,respValue
def moveToLevel(myNodeId, myEpId, mySendMode, myLevel=100, myDuration=0):
    """
        myDuration resolution is 0.1s
        AT+LCMVTOLEV:19EA,01,0,1,1A,0001
        DFTREP:19EA,01,0008,04,00
        
        0x04 in DFTREP above is the command ID.
    """
    onOff = 1
    assert myLevel>=0
    assert myLevel<=100
    dim = round(myLevel * 255 /100)
    myMsg = 'AT+LCMVTOLEV:{},{},{},{},{:02x},{:04x}'.format(myNodeId,myEpId,mySendMode,onOff,dim,myDuration)

    if mySendMode==0:
        expectedResponses = ["DFTREP:{},{},{},{},(..)".format(myNodeId,myEpId,'0008','04')]
    else:
        expectedResponses = ['OK']
        
    respState,respCode,respValue = sendCommand(myMsg, expectedResponses)
    return respState,respCode,respValue
def colourTemperature(myNodeId,myEp,mySendMode=0,myColourTemp=4000,myDuration=0):
    """ Set the given colour temperature for the bulb
    
        AT+CCMVTOCT:0EE1,01,0,0000,0000
    """
    mireds = round(1000000/myColourTemp)
    myMsg='AT+CCMVTOCT:{},{},{},{:04x},{:04x}'.format(myNodeId,myEp,mySendMode,mireds,myDuration)
    if mySendMode==0:
...
def getAtr(clustId,attrId):
    ...
    # ATTR:1DBEFCAE
    # \A at start of the regex expression forces a match at the beginning of the string
    expectedResponse=['\AATTR:']
    respState,respCode,respValue = sendCommand(myMsg, expectedResponse)
...
def pjoin(duration):
    """
    """
    myMsg="AT+PJOIN:{:02x},FFFC".format(duration)
    expectedResponse=['OK']
    respState, respCode, respValue = sendCommand(myMsg, expectedResponse)    
    return respState,respCode,respValue 

def pjoin_old(devTypeList):
...
def setMode(myNodeId,myEp,myMode,myEpType,mySetpointFloat=None,myDuration=1):
    ...
    if myDuration not in range(0,7):
        respState=False
        respValue='ERROR: Boost Duration must be in the range 1-6. Or 0 for 2min getTelemetry Boost'
        return respState,None,respValue
    else:
...
def getInitialData(nodeId, nodeEp, fastPoll=True, printStatus=False):
    ...
    # Get the simple descriptor for each endpoint
    for ep in endpoints:
        # Get the simple descriptor
        # {'DeviceID': '0009v00', 'ProfileID': '0104', 'OutCluster': ['0019'], 'EP': '09', 'InCluster': ['0000', '0001', '0002', '0003', '0004', '0005', '0006', '0009', '0402', '0702']}
        respState,respCode,respValue=getSimpleDesc(nodeId, ep)
        if respState and respCode == zcl.statusCodes['SUCCESS']:
            print("\nSimple Descriptor, Endpoint={}:".format(ep))
            for f in ['ProfileID','DeviceID','InCluster','OutCluster']:
                if f=='DeviceID':
                    deviceType=zcl.getDeviceType(respValue[f])
                    if deviceType==None:
                        print("{:10} = {}. ****** DEVICE TYPE NOT LISTED IN ZIGBEE CLUSTER LIBRARY.  PLEASE ADD IT. (Page 49 HA Spec)".format(f,respValue[f]))
                    else:
                        print("{:10} = {}. {}".format(f,respValue[f],deviceType))
                else:    
                    print("{:10} = {}".format(f,respValue[f]))
        else:
            print("getSimpleDesc Error".format(respValue))
            exit()
    ...
    if respState and respCode==zcl.statusCodes['SUCCESS']:
        ...
        if printStatus: print('\nNode EUI={0}, nodeID={1}'.format(nodeEUI,nodeId))
    else:
        print("getEUI error. {}".format(respValue))
        exit()
...
def dimmingTest(myNodeId,myEpId,minLevel,maxLevel,duration,iterations):
    ...
    while True:
        for l in range(0,10):
            ...
            durationHex = "{:04x}".format(duration*10)
            moveToLevel(myNodeId, myEpId, levelHex, durationHex)
...
def joinDevices():
    ...
    resp=waitForMessage("SED:", 500)
    return resp.split(',')[1]  
    
def setBind(nodeId,nodeEp,params,myCluster):
    """ Set binding on given cluster
    
    """
    mySrcAddr = params['nodeEUI']
    myDstAddr = params['controllerEUI']
    mySrcEp = nodeEp
    myDstEp = '01'
    myClusterId,_ = zcl.getClusterNameAndId(myCluster)
    respState,respCode,respValue = setBinding(nodeId, mySrcAddr, mySrcEp, myClusterId, myDstAddr, myDstEp)
    if not respState or respCode != zcl.statusCodes['SUCCESS']:
        print("ERROR: setBind {},{}".format(respCode,respValue))
        exit()
    return
def setAttrReport(nodeId,nodeEp,params,myCluster,myAttribute,minRep,maxRep,changeRep):
    """ Attempt to set an attribute reporting configuration.
    
    """    
    myClusterId,_ = zcl.getClusterNameAndId(myCluster)
    attrId,_,_ = zcl.getAttributeNameAndId(myCluster, myAttribute)
    
    respState,respCode,respValue = setAttributeReporting(nodeId, nodeEp, myClusterId, attrId, minRep, maxRep, changeRep)
    if not respState or respCode != zcl.statusCodes['SUCCESS']:
        print("ERROR: setAttrVal {},{}".format(respCode,respValue))
        exit()
    
    return
def redBMtest1(nodeId,nodeEp,params):
    """ redBMTest1
    
        Set a binding on the pollControl cluster.
        Set an attribute report configuration 
        Wait for a check-in
        
        Set a binding on the powerConfig cluster
        Set an attribute report configuration
        
        Wait to see if temperature reports stop
    
    """
    # Wait for a device announce from a SED
    myMsg='AT+PJOIN:3c,fffc'
    txQueue.put(myMsg)
    ...
    # Set binding on poll control cluster
    print("Setting binding for poll control cluster")
    setBind(nodeId,nodeEp,params,'Poll Control Cluster')
    print("Setting attribute report for checkInInterval")
    setAttrReport(nodeId,nodeEp,params,'Poll Control Cluster','checkInInterval','0001','0000','00000000') 
    print("Reading attribute")
    _,_,respValue= getAttribute(nodeId, nodeEp, '0020', '0000', 'server')
    print(respValue)

    # Set attribut reporting config on checkInInterval attribute

    # Wait for a check-in
    print("Waiting for a check-in")
    timeout = 130 # Seconds (a bit longer than 2mins)
    waitForMessage("CHECKIN:{},{}".format(nodeId,nodeEp), timeout)
    print('Check-in received')
   
    # Set binding on power config cluster
    setBind(nodeId,nodeEp,params,'Power Configuration Cluster')
    minRep='0001'
    maxRep='0000'
    changeRep='01'
    setAttrReport(nodeId,nodeEp,params,'Power Configuration Cluster','batteryVoltage',minRep,maxRep,changeRep)
    
    print('Test Complete. Check BM for red LED.')
    return
