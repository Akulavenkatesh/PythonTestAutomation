...
nodeTypes = {'hub':'http://alertme.com/schema/json/node.class.hub.json#',
               'tui':'http://alertme.com/schema/json/node.class.thermostatui.json#',
               'bm':'http://alertme.com/schema/json/node.class.thermostat.json#'}

hubDescriptor = 'hardwareVersion'
...
#                     return 0
    ...
    def printAccount(self):
        ...
        return myString
""" Helper Methods """
def parseAttributeEntry(node, myKey, mySubKey='reportedValue'):
    """
    """
    if 'attributes' in node:
        if myKey in node['attributes']:
            return node['attributes'][myKey][mySubKey]
        else:
            return None
    else:
        return None
...
""" Account Class """
class account():
    def __init__(self,session,targetUser=None,hubText=None):
        self.username = targetUser
        self.hubText = hubText
        self.apiURL = session.apiURL
        self.headers = buildImpersonateSessionHeaders(session, targetUser, hubText)
        self.uuid = self.headers['X-Omnia-User']
        self.nodes = None
        
        # Hub variables
        self.accountFound=False
        self.hubFound=False
        self.hubOnline=False
        self.hubType = None
        self.hubSoftwareVersion = None
        self.hubNodeId = None
        
        # If account found the proceed with getNodes
        # parse the response to populate the instance vars
        if self.uuid==None:
            return
        else:
            self.accountFound=True
        ...
        return
    def update(self):
        """ Make a call to /Nodes and update class instance parameters
        
        """
        nodes,nodesSuccess = getNodes(self)
        print(json.dumps(nodes.json(),indent=4))
            
        # Parse nodes response to build a list of devices, Node IDs and FW versions.
        self.devices = []
        if self.accountFound and nodesSuccess:
            self.nodes = nodes.json()
            for n in self.nodes['nodes']:
                nodeType = n['nodeType']
                if nodeType == nodeTypes['hub']:
                    self.hubFound = True
                    self.hubType = parseAttributeEntry(n, hubDescriptor)
                    self.hubSoftwareVersion = parseAttributeEntry(n, 'softwareVersion')
                    self.hubSoftwareInstallationState = parseAttributeEntry(n,'softwareInstallationState')
                    self.hubNodeId = n.get('id')
                    self.hubOnline = parseAttributeEntry(n, 'presence')
                else:
                    dev = device(n)
                    self.devices.append(dev)
        
        self.hubStatusString = ''
        self.hubOK = False
        if not self.accountFound:
            self.hubStatusString = 'ACCOUNT_NOT_FOUND'
        elif not self.hubFound:
            self.hubStatusString = 'HUB_NOT_FOUND'
        elif not self.hubOnline:
            self.hubStatusString = 'HUB_OFFLINE'
        else:
            self.hubStatusString = 'HUB_ONLINE'
            self.hubOK = True
        return
    def printDevices(self):
        """ Form a string with details of each device on the account
            We use the parent==hub node to filter out device sub nodes (e.g. BM endpoints)
            
        """
        hubNodeId = self.hubNodeId
        myString = "{},{},{},{}".format(self.username,self.hubType,self.hubSoftwareVersion,self.hubStatusString)
        for dev in self.devices:
            if dev.parentNodeId == hubNodeId:
                devType = dev.type
                if devType==None:
                    devType='None'
                
                devSoftwareVersion = dev.softwareVersion
                if devSoftwareVersion==None:
                    devSoftwareVersion='None'
                    
                devPresence = dev.presence
            
                myString = myString + ',' + devType + ',' + devSoftwareVersion + ',' + devPresence
        return myString
class device():
    def __init__(self,node):
        self.type = parseAttributeEntry(node, deviceDescriptor)
        self.softwareVersion = parseAttributeEntry(node, 'softwareVersion')
        self.softwareInstallationState = parseAttributeEntry(node, 'softwareInstallationState')
        self.nodeId = node.get('id')
        self.presence = parseAttributeEntry(node, 'presence')
        self.parentNodeId = node.get('parentNodeId')
        return
 
def buildImpersonateSessionHeaders(session,targetUser,hubText):
    """
    """
    impersonateHeaders = dict(session.mgntHeaders)
    # Set the header for the given user (for impersonate mode)
    if session.server=='isopLivev6':
        if hubText == None:
            print("Server is {}. HubText={}.  We require a hub text on this server to find the uuid.".format(session.server,hubText))
            exit()
    
    impersonateHeaders['X-Omnia-User']=getUuid(session,targetUser,hubText)
    return impersonateHeaders
def getUuid(session,targetUser,hubText):
    """ Returns the uuid of the target user.  Uses hub text if present.
    
    """
    # Use hub text if given else username
    if hubText==None:
        url = session.apiURL + '/omnia/users?name={}'.format(targetUser)
    else:
        url = session.apiURL + '/omnia/users?hubText={}'.format(hubText)
        
    r = requests.get(url,headers=session.mgntHeaders) 
        
    if r.status_code!=200:
        print("ERROR in getUuid(): ",r.status_code,r.reason,r.url,r.text)
        exit()
