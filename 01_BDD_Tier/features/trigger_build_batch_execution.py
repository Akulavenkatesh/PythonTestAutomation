...
class BatchTrigger():
        ...
        try: 
            ...
            if len(oKitList.keys()) > 0:
                ...
                for oPKey in sorted(oKitPriority.keys()):
                    self.update_gloabalVar_json(self.current_batch_id, oKitPriority[oPKey])
                    ...
                    self.Batch_Execution_Summary_KitAddLink(oKitPriority[oPKey], Main_Client, Second_Client, User_ID, Kit_Setup, Test_Suite)
    ...
    def Batch_Execution_Summary_Initialize(self):
        ...
        try:             
            ...
            oFileWriter = open(self.strExecSummaryHTMLFilePath, 'x')
            ...
            print("I/O error({0}): {1} in  Batch summary initialize".format(e.errno, e.strerror), traceback.format_exc().replace('File', '$~File'))
            
            
    def Batch_Execution_Summary_KitAddLink(self, Kit_ID, Main_Client, Second_Client, User_ID, Kit_Setup, Test_Suite):
        try:
            ...
            oFileWriter.write("<td class='justified'><a href='" + './Kit_Results/' + strFilePath + "' target='about_blank'>" + str(Kit_ID) + "</a></td>\n")
