...
class UIClass(Frame):
    ...
    def load_kit_details(self, event):
        ...
        if current_batch_id != "" and current_kit_id != "":
            ...
            self.api_selected_item_event(None)
            self.oMainClient.set(oKitDetails['mainClient']['name'])
            self.load_app_details(None)
            self.oAppVersion.set(oKitDetails['currentAppVersion'])
            ...
            if 'IOS' in oKitDetails['mainClient']['name'].upper():
                self.oAppFilePath.set(oKitDetails['mainClient']['appFileName'])
                self.oDeviceName.set(oKitDetails['mainClient']['deviceName'])
                self.oUDID.set(oKitDetails['mainClient']['udid'])
                self.oOSVersion.set(oKitDetails['mainClient']['platformVersion'])
            elif 'ANDROID' in oKitDetails['mainClient']['name'].upper():
                self.oAppFilePath.set(oKitDetails['mainClient']['appFileName'])
                self.oDeviceName.set(oKitDetails['mainClient']['deviceName'])
                self.oOSVersion.set(oKitDetails['mainClient']['platformVersion'])
            elif 'WEB' in oKitDetails['mainClient']['name'].upper():
                self.oBrowserName.set(oKitDetails['mainClient']['browserName'])
                self.oURL.set(oKitDetails['mainClient']['loginURL'])
    ...
    def update_kit_details(self):
        ...
        if not (current_batch_id == "" and current_kit_id == ""):
            ...
            if self.session.statusCode != 200:
                ...
                oKitDetails['currentAppVersion'] = self.AppVersion.get()
                ...
                oKitDetails['mainClient'] = {}
                if 'IOS' in self.MainClient.get().upper():
                    oKitDetails['mainClient']['appFileName'] = self.AppFilePath.get()
                    oKitDetails['mainClient']['deviceName'] = self.DeviceName.get()
                    oKitDetails['mainClient']['name'] = self.MainClient.get()
                    oKitDetails['mainClient']['udid'] = self.UDID.get()
                    oKitDetails['mainClient']['platformVersion'] = self.OSVersion.get()
                    ...
                    oKitDetails['mainClient']['name'] = self.MainClient.get()
                    oKitDetails['mainClient']['browserName'] = self.BrowserName.get()
                    oKitDetails['mainClient']['loginURL'] = self.URL.get()
